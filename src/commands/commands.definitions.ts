...
function makeDef(name: string, translateKey: string, icon: string, uiOnly: boolean, partOfPage: boolean, more: any) {
...
$2sxc._commands.definitions.create = function (cmdSpecs) {
  ...
  addDef(makeDef("replace", "Replace", "replace", false, true, {
    showCondition(settings) { return settings.useModuleList; }
  }));
