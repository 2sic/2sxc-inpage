...
let tools = twoSxc._toolbarManager.buttonHelpers = {
  ...
  buildFullDefinition: (unstructuredConfig, allActions, instanceConfig, moreSettings) => {
    let fullConfig = tools.ensureDefinitionTree(unstructuredConfig, moreSettings);
  ...
  ensureDefinitionTree: (original, moreSettings) => {
  ...
  expandButtonGroups: (fullSet, actions) => { //, itemSettings) {
    ...
    for (let g = 0; g < fullSet.groups.length; g++) {
      ...
      // fix all the buttons
      let btns = fullSet.groups[g].buttons;
  ...
  expandButtonConfig: (original, sharedProps) => {
  ...
  removeDisableButtons: (full, config) => {
    let btnGroups = full.groups;
    for (let g = 0; g < btnGroups.length; g++) {
      let btns = btnGroups[g].buttons;
  ...
  addDefaultBtnSettings: (btn, group, groups, actions) => {
    for (let d = 0; d < tools.btnProperties.length; d++)
  ...
  evalPropOrFunction: (propOrFunction, settings, config, fallback) => {
