...
import { addItem, removeFromList, changeOrder, publishId, publish } from '../../contentBlock/actions';
...
export class Commands {
  commandList = cmds;
  list = getList(this.editContext); // hash - table of action definitions, to be used a list()["action - name"]
  get: (name: string) => {}; // a specific action definition

  constructor(private editContext: DataEditContext) {  }
}

export function getList(editContext: DataEditContext) {
...
function create(cmdSpecs: CmdSpec): void {
...
function addDef(def: CommandDefinition): void {
  cmds[def.name] = def;
}

// quick helper so we can better debug the creation of definitions
function makeDef(name: string, translateKey: string, icon: string, uiOnly: boolean, partOfPage: boolean, more: Definition): CommandDefinition {
  ...
  const newDefinition: CommandDefinition = {
    name: name,
    buttonConfig: btnConfig as ButtonConfig, // stv: hack: fix this, buttonConfig is not expanded
  };
